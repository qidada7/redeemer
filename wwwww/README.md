...
#Install Gulp and semantic
This app is using Semantic-Ui as the front-end framework, so we need to install it properly.

First, we need to install Gulp as global.

```
npm install -g gulp
```

Then, we need to install semantic. There is a bug in semantic-ui, so you need to install 10.11.0 version of node.js to installed
semantic-ui.
